...
- (void)initSetup{
    ...
    _shapeLayer.frame = CGRectMake(0, 0, UIScreenBounds.size.width, _minimalHeight);
    ...
    //布局控制点
    [self layoutControlPoints:_minimalHeight waveHeight:0.0 locationX:UIScreenBounds.size.width/2];
...
-(void) panGestureDidMove:(UIPanGestureRecognizer *)pan
{
    ...
    {
        CGFloat maxF = [pan translationInView:self].y;
        ...
        CGFloat locationX = [pan translationInView:self].x + pan.view.center.x;  //视图上手指滑动的x坐标，也就是波浪的顶部
        NSLog(@"+++++++%f",locationX);
...
{
    CGFloat width = UIScreenBounds.size.width;
