...
@interface ExampleTableViewCell()<UITableViewDelegate,UITableViewDataSource,SWTableViewCellDelegate>
{
    NSArray *_sections;
...
- (id)init{
    ...
    self.tableView = [[UITableView alloc]initWithFrame:UIScreenBounds style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.useCustomCells){
        ...
        UMTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"UMTableViewCell"];
       
        if (cell == nil) {
            
            cell = [[[NSBundle mainBundle] loadNibNamed:@"UMTableViewCell" owner:self options:nil]lastObject];
            
            //添加内容项 设置每个button的宽度
            [cell setLeftUtilityButtons:[self leftButtons] WithButtonWidth:40.0f];
